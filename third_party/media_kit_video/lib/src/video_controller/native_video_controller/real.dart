...
class NativeVideoController extends PlatformVideoController {
  ...
  NativeVideoController._(
    super.player,
    super.configuration,
  )   : width = usesScaledVideoOutput(configuration.scale)
            ? null
            : configuration.width,
        height = usesScaledVideoOutput(configuration.scale)
            ? null
    ...
    videoParamsSubscription = player.stream.videoParams.listen(
      (event) => lock.synchronized(() async {
        ...
        final output = scaledVideoOutputSize(
          width: sourceWidth,
          height: sourceHeight,
          scale: configuration.scale,
        );

        if (videoParamsWidth == output.width &&
            videoParamsHeight == output.height) {
          return;
        }

        videoParamsWidth = output.width;
        videoParamsHeight = output.height;

        // A caller-selected fixed size takes precedence until setSize(null)
        // restores the scaled dimensions derived from VideoParams.
