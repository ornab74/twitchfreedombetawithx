...
struct _VideoOutput {
  ...
  gint64 pixel_buffer_width;
  gint64 pixel_buffer_height;
...
static void video_output_dispose(GObject* object) {
  ...
  g_atomic_int_set(&self->destroyed, TRUE);
...
static void video_output_init(VideoOutput* self) {
  ...
  self->pixel_buffer_width = 0;
  self->pixel_buffer_height = 0;
  self->handle = NULL;
  self->render_context = NULL;
  self->width = 0;
  self->height = 0;
...
VideoOutput* video_output_new(FlTextureRegistrar* texture_registrar,
  ...
  if (!hardware_acceleration_supported) {
    ...
    self->render_buffer = g_new0(guint8, SW_RENDERING_PIXEL_BUFFER_SIZE);
    self->texture_gl = NULL;
            ...
            [](void* data) {
              ...
              if (g_atomic_int_get(&self->destroyed) ||
                  !g_atomic_int_compare_and_exchange(
                      &self->software_frame_pending, 0, 1)) {
                return;
              }
              GTask* task = g_task_new(
                  self, NULL,
                  [](GObject* source, GAsyncResult*, gpointer) {
                    VideoOutput* self = VIDEO_OUTPUT(source);
                    if (!g_atomic_int_get(&self->destroyed)) {
                      fl_texture_registrar_mark_texture_frame_available(
                          self->texture_registrar,
                          FL_TEXTURE(self->texture_sw));
                    }
                    g_atomic_int_set(&self->software_frame_pending, 0);
                  },
                  NULL);
              g_task_run_in_thread(
                  task,
                  [](GTask* task, gpointer source, gpointer, GCancellable*) {
                    VideoOutput* self = VIDEO_OUTPUT(source);
                    if (g_atomic_int_get(&self->destroyed)) {
                    ...
                    if (width > 0 && height > 0) {
                      ...
                      mpv_render_param params[]{
                          ...
                          {MPV_RENDER_PARAM_SW_POINTER, self->render_buffer},
                          {MPV_RENDER_PARAM_INVALID, (void*)0},
                      };
                      mpv_render_context_render(self->render_context, params);
                      guint8* completed = self->render_buffer;
                      self->render_buffer = self->pixel_buffer;
                      self->pixel_buffer = completed;
...
gboolean video_output_acquire_software_frame(VideoOutput* self,
                                             const guint8** buffer,
                                             gint64* width,
                                             gint64* height) {
  // A blocking lock here can freeze all Flutter painting while mpv converts a
  // software frame. Reuse the previously uploaded GL texture instead and let
  // the next completed-frame notification try again.
  if (!g_mutex_trylock(&self->mutex)) return FALSE;
  if (self->pixel_buffer == NULL || self->pixel_buffer_width <= 0 ||
      self->pixel_buffer_height <= 0) {
    g_mutex_unlock(&self->mutex);
    return FALSE;
  }
  *buffer = self->pixel_buffer;
  *width = self->pixel_buffer_width;
  *height = self->pixel_buffer_height;
  return TRUE;
}

void video_output_release_software_frame(VideoOutput* self) {
  g_mutex_unlock(&self->mutex);
}
