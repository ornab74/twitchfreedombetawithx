...
static gboolean invoke_resize_on_platform_thread(gpointer user_data) {
  IdleCallbackData* data = (IdleCallbackData*)user_data;
  fl_method_channel_invoke_method(data->channel, "VideoOutput.Resize",
                                  data->result, NULL, NULL, NULL);
  return G_SOURCE_REMOVE;
}

static void dispose_idle_callback_data(gpointer user_data) {
  IdleCallbackData* data = (IdleCallbackData*)user_data;
  g_object_unref(data->channel);
  fl_value_unref(data->result);
  g_free(data);
}
    ...
    video_output_manager_create(
        ...
        [](gint64 id, gint64 width, gint64 height, gpointer context) {
          ...
          g_idle_add_full(G_PRIORITY_DEFAULT_IDLE,
                          invoke_resize_on_platform_thread, idle_data,
                          dispose_idle_callback_data);
        },
        data);
